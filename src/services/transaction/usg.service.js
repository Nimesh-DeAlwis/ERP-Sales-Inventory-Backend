...
const { 
    ...
    USGDetail, 
    USGSearchCriteria 
} = require('../../models/transaction/usg.model');
...
class USGService {
    ...
    async getNextRunNo(type, comCode, locCode) {
        try {
            const result = await executeStoredProcedure('sp_USG_GetNextRunNo', [
    ...
    async getTransactionSetup(type) {
        try {
            const result = await executeStoredProcedure('sp_USG_GetSetup', [
                { name: 'type', value: type, type: sql.NVarChar(3) }
            ]);
    ...
    // Create Usage Note
    async createUSG(headerData, details, currentUser) {
        try {
            // Ensure run number is set
            if (!headerData.runNo || headerData.runNo.trim() === '') {
                const locCode = headerData.locFrom || headerData.setupLoc || headerData.logLocation;
                headerData.runNo = await this.getNextRunNo(headerData.type, headerData.comCode, locCode);
            }

            // Create header
            await executeStoredProcedure('sp_USG_CreateHeader', [
            ...
            for (const detail of details) {
                await executeStoredProcedure('sp_USG_CreateDetail', [
            ...
            return {
                success: true,
                message: 'Usage Note created successfully',
                data: {
            ...
            console.error('Error in createUSG:', error);
            throw error;
        }
    }

    // Update Usage Note
    async updateUSG(header, details, currentUser) {
        ...
        try {
            ...
            for (const detail of details) {
                await request.query(`
                    INSERT INTO T_TBLUSGDETAILS (
                        DET_TYPE, DET_RUNNO, DET_COMCODE, DET_LINENO, 
                        DET_LOCFROM, DET_LOCTO, DET_PROCODE,
                        DET_STOCKCODE, DET_PRODESC, DET_UNIT,
                        DET_UNITQTY, DET_CPRICE, DET_SPRICE, 
                        DET_AMOUNT, CR_DATE, CR_BY
                    ) VALUES (
                        '${header.type}', '${header.runNo}', '${header.comCode}', ${detail.lineNo},
                        '${header.locFrom}', '${header.locTo || ''}', '${detail.proCode}',
                        '${detail.stockCode}', '${detail.proDesc}', '${detail.unit}',
                        ${detail.unitQty}, ${detail.cPrice}, ${detail.sPrice},
                        ${detail.amount}, GETDATE(), '${currentUser}'
                    )
                `);
            }
            ...
            return {
                success: true,
                message: 'Usage Note updated successfully',
                data: { runNo: header.runNo }
            };
        } catch (error) {
            if (transaction) await transaction.rollback();
            console.error('Error in updateUSG:', error);
            throw error;
        }
    }

    // Process Usage Note (Deduct from Inventory)
    async processDocument(type, runNo, comCode, locFrom, currentUser) {
        try {
            const result = await executeStoredProcedure('sp_USG_Process', [
                { name: 'type', value: type, type: sql.NVarChar(3) },
            ...
            return {
                success: true,
                message: 'Usage Note processed successfully! Inventory updated.',
    ...
    async getDocumentByNo(type, runNo, comCode, locFrom) {
        try {
            const result = await executeStoredProcedure('sp_USG_GetByDocNo', [
                { name: 'type', value: type, type: sql.NVarChar(3) },
                { name: 'runNo', value: runNo, type: sql.NVarChar(13) },
                { name: 'comCode', value: comCode, type: sql.NVarChar(5) },
                { name: 'locFrom', value: locFrom, type: sql.NVarChar(5) }
            ...
            return {
                header: new USGHeader(result.recordsets[0][0]),
                details: result.recordsets[1].map(row => new USGDetail(row))
            };
        } catch (error) {
    ...
    async searchDocuments(criteria) {
        try {
            const searchCriteria = new USGSearchCriteria(criteria);

            const result = await executeStoredProcedure('sp_USG_Search', [
            ...
            const documents = result.recordset.map(row => new USGHeader(row));
    ...
    async generatePDF(type, runNo, comCode, locFrom) {
        try {
            ...
            // Header
            doc.fontSize(20).text('USAGE NOTE', { align: 'center' });
            doc.moveDown();
            ...
            // Footer Note
            y += 30;
            doc.fontSize(8).text('Note: This document records products used/consumed in the outlet.', 50, y, { align: 'center' });
