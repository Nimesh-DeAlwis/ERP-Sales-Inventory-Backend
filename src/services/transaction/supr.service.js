...
const { 
    ...
    SUPRDetail, 
    SUPRSearchCriteria 
} = require('../../models/transaction/supr.model');
...
class SUPRService {
    ...
    async getNextRunNo(type, comCode, locCode) {
        try {
            const result = await executeStoredProcedure('sp_SUPR_GetNextRunNo', [
                { name: 'type', value: type, type: sql.NVarChar(10) },
    ...
    async getTransactionSetup(type) {
        try {
            // For now, return hardcoded setup since stored procedure may not exist
            return {
                success: true,
                data: {
                    txSupplier: 1, // SUPR requires supplier
                    txCustomer: 0,
                    txChangePrice: 1, // Allow price changes
                    defaultLocFrom: '',
                    defaultLocTo: '',
                    locations: [] // Will be populated from location service
                }
            };
        } catch (error) {
    ...
    async searchGRNDocuments(searchText, comCode, locFrom) {
        try {
            const result = await executeStoredProcedure('sp_SUPR_SearchGRN', [
            ...
            console.error('Error searching GRN documents:', error);
            throw error;
        }
    }

    // Get GRN document details
    async getGRNDocument(grnType, grnRunNo, comCode, locFrom) {
        try {
            const result = await executeStoredProcedure('sp_SUPR_GetGRN', [
                { name: 'grnType', value: grnType, type: sql.NVarChar(4) },
                { name: 'grnRunNo', value: grnRunNo, type: sql.NVarChar(13) },
                { name: 'comCode', value: comCode, type: sql.NVarChar(5) },
                { name: 'locFrom', value: locFrom, type: sql.NVarChar(5) }
            ]);
            ...
            console.error('Error getting GRN document:', error);
            throw error;
        }
    }

    // Create Supplier Return
    async createSUPR(headerData, details, currentUser) {
        try {
            // Create header
            await executeStoredProcedure('sp_SUPR_CreateHeader', [
                { name: 'type', value: headerData.type, type: sql.NVarChar(4) },
            ...
            for (const detail of details) {
                await executeStoredProcedure('sp_SUPR_CreateDetail', [
            ...
            console.error('Error in createSUPR:', error);
            throw error;
        }
    }

    // Update Supplier Return
    async updateSUPR(header, details, currentUser) {
        ...
        try {
            ...
            console.error('Error in updateSUPR:', error);
            throw error;
        }
    }

    // Process Supplier Return (Deduct from Inventory)
    async processDocument(type, runNo, comCode, locFrom, currentUser) {
        try {
            const result = await executeStoredProcedure('sp_SUPR_Process', [
                { name: 'type', value: type, type: sql.NVarChar(4) },
            ...
            return {
                success: true,
                message: 'Supplier Return processed successfully! Inventory updated.',
    ...
    async getDocumentByNo(type, runNo, comCode, locFrom) {
        try {
            const result = await executeStoredProcedure('sp_SUPR_GetByDocNo', [
                { name: 'type', value: type, type: sql.NVarChar(4) },
                { name: 'runNo', value: runNo, type: sql.NVarChar(13) },
                { name: 'comCode', value: comCode, type: sql.NVarChar(5) },
                { name: 'locFrom', value: locFrom, type: sql.NVarChar(5) }
            ...
            return {
                header: new SUPRHeader(result.recordsets[0][0]),
                details: result.recordsets[1].map(row => new SUPRDetail(row))
            };
        } catch (error) {
    ...
    async searchDocuments(criteria) {
        try {
            const searchCriteria = new SUPRSearchCriteria(criteria);

            const result = await executeStoredProcedure('sp_SUPR_Search', [
                { name: 'searchText', value: searchCriteria.searchText, type: sql.NVarChar(100) },
                { name: 'type', value: searchCriteria.type, type: sql.NVarChar(4) },
            ...
            const documents = result.recordset.map(row => new SUPRHeader(row));
    ...
    async generatePDF(type, runNo, comCode, locFrom) {
        try {
            ...
            // Header
            doc.fontSize(20).text('SUPPLIER RETURN NOTE', { align: 'center' });
            doc.moveDown();
            ...
            doc.text(`GRN Reference: ${header.refNo || 'N/A'}`, 50, 145);
...
module.exports = new SUPRService(); 